...
- `GpxExporter` / `GpxParser` write and read GPX; exports share via `FileProvider` or bulk-write to
  ...
  the whole history as one gzipped JSON file, which is also what the companion viewer in `site/src/viewer/`
  reads.
...
- Single device / single user: no multi-device sync (server upload to e.g. Dawarich/OwnTracks is a
  possible future addition). `site/src/viewer/` renders a backup file in the browser instead — the same data,
